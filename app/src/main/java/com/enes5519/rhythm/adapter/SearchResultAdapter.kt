...
//        }
        ...
        val builder = NotificationCompat.Builder(context.applicationContext, notificationChannelId).apply{
        ...
        CoroutineScope(Dispatchers.IO).launch {
            NotificationManagerCompat.from(context).apply {
                ...
                withContext(Dispatchers.Main){
                    builder.setContentText(context.getString(R.string.downloading_music)).setProgress(0,0,true)
                    notify(notificationId, builder.build())
                }
                    ...
                    withContext(Dispatchers.Main) {
                        when (it) {
                            is DownloadResult.Success -> {
                                ...
                                delay(1000) // hack
                                builder.setProgress(0, 0, false).setContentText(context.getString(R.string.music_downloaded)).setOngoing(false).setCategory(
                                    NotificationCompat.CATEGORY_MESSAGE)
                                notify(notificationId, builder.build())
                            }

                            is DownloadResult.Error -> {
                                builder.setProgress(0, 0, false).setContentText(context.getString(R.string.error_download)).setOngoing(false).setCategory(
                                    NotificationCompat.CATEGORY_MESSAGE)
                                notify(notificationId, builder.build())
                            }
